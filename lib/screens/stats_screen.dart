...
class _StatsScreenState extends ConsumerState<StatsScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    if (total == 0) return Scaffold(backgroundColor: kBackground, body: _buildEmpty());
              ...
              title: FittedBox(
                ...
                child: Column(crossAxisAlignment: CrossAxisAlignment.start, mainAxisSize: MainAxisSize.min, children: [
                  Text('FIELD LOGBOOK // METRICS',
                      style: GoogleFonts.jetBrainsMono(color: kAccent, fontSize: 8.sp, fontWeight: FontWeight.w800, letterSpacing: 1.8)),
                  SizedBox(height: 2.h),
                  Text('Analytics', style: GoogleFonts.sora(color: kPrimaryText, fontSize: 28.sp, fontWeight: FontWeight.w700, height: 1.0)),
                ]),
              ),
            ),
          ),
          ...
          SliverPadding(
            ...
            sliver: SliverList(
              delegate: SliverChildListDelegate([
                ...
                _SummaryStrip(total: total, typeCount: toolCounts.length, scaleCount: scaleCounts.length),
                ...
                _InteractiveBarList(
                  title: 'Tool Types',
                  data: toolCounts.map((key, value) => MapEntry(key.label, _BarData(value, getToolTypeColor(key)))).entries,
                  total: total,
                ),
                SizedBox(height: 36.h),
                ...
                _InteractiveBarList(
                  title: 'Scale Systems',
                  data: scaleCounts.map((key, value) => MapEntry(key.label, _BarData(value, getScaleSystemColor(key)))).entries,
                  total: total,
                ),
                SizedBox(height: 36.h),
  ...
  Widget _sectionLabel(String label) {
    return Row(
      children: [
        ...
        SizedBox(width: 8.w),
        Text(label,
            style: GoogleFonts.jetBrainsMono(color: kSecondaryText, fontSize: 10.sp, fontWeight: FontWeight.w700, letterSpacing: 1.2)),
      ],
    );
  }
  ...
  Widget _buildEmpty() {
    return Center(
      child: Column(mainAxisSize: MainAxisSize.min, children: [
        ...
        SizedBox(height: 16.h),
        Text('NO DATA YET.',
            style: GoogleFonts.jetBrainsMono(color: kSecondaryText, fontSize: 13.sp, fontWeight: FontWeight.w600, letterSpacing: 1.0)),
        SizedBox(height: 6.h),
...
class _SummaryStrip extends StatelessWidget {
  ...
  const _SummaryStrip({required this.total, required this.typeCount, required this.scaleCount});
        ...
        child: Column(children: [
          TweenAnimationBuilder<double>(
            ...
            builder: (context, val, child) {
              return Text(val.toInt().toString(), style: GoogleFonts.jetBrainsMono(color: color, fontSize: 26.sp, fontWeight: FontWeight.w800));
            },
          ),
          SizedBox(height: 6.h),
          Text(label, style: GoogleFonts.inter(color: kSecondaryText, fontSize: 10.sp, fontWeight: FontWeight.w600), textAlign: TextAlign.center, maxLines: 1, overflow: TextOverflow.ellipsis),
        ]),
      ),
    );
  }
...
class _InteractivePrecisionDial extends StatefulWidget {
  ...
  @override
  State<_InteractivePrecisionDial> createState() => _InteractivePrecisionDialState();
}
                ...
                return GestureDetector(
                  onTapUp: (details) {
                    ...
                    if (dist >= innerR - 10 && dist <= outerR + 20) {
                      double tapAngle = (math.atan2(dy, dx) + math.pi / 2) % (2 * math.pi);
                      ...
                      if (!found) setState(() => _selected = null);
                    } else {
  ...
  Widget _buildReadout(List<MapEntry<ConditionGrade, int>> entries) {
    if (_selected == null) {
      return Container(
        ...
        child: Center(child: Text('TAP A SECTOR FOR DETAILS', style: GoogleFonts.jetBrainsMono(color: kSecondaryText, fontSize: 11.sp, letterSpacing: 1.0, fontWeight: FontWeight.w600))),
      );
    }
    ...
    return Container(
      ...
      child: Row(
        ...
        children: [
              ...
              children: [
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      Text(_selected!.label.toUpperCase(), style: GoogleFonts.sora(color: kPrimaryText, fontSize: 14.sp, fontWeight: FontWeight.w700), maxLines: 1, overflow: TextOverflow.ellipsis),
                      SizedBox(height: 4.h),
                      Text('$count INSTRUMENT${count == 1 ? '' : 'S'}', style: GoogleFonts.jetBrainsMono(color: kSecondaryText, fontSize: 10.sp, fontWeight: FontWeight.w600)),
                    ],
                  ),
                ),
              ],
          ...
          Text('$pct%', style: GoogleFonts.jetBrainsMono(color: color, fontSize: 22.sp, fontWeight: FontWeight.w800)),
        ],
      ),
    );
  }
...
class _DialPainter extends CustomPainter {
  ...
  _DialPainter({required this.entries, required this.total, required this.selected, required this.animation});
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final tickPaint = Paint()..color = kOutline..strokeWidth = 1.0;
    for (int i = 0; i < 72; i++) {
      ...
      canvas.drawLine(
        ...
        Offset(cx + math.cos(angle) * tickOuter, cy + math.sin(angle) * tickOuter),
    ...
    for (final e in entries) {
      ...
      final paint = Paint()
        ..color = getConditionColor(e.key).withAlpha(selected == null || isSelected ? 255 : 100)
      ...
      if (isSelected) {
        ...
        canvas.drawArc(
          Rect.fromCircle(center: Offset(cx, cy), radius: (segOuterR + innerR) / 2),
          startAngle, math.max(0.001, sweep - 0.04), false, shadowPaint,
        );
      }
      ...
      canvas.drawArc(
        Rect.fromCircle(center: Offset(cx, cy), radius: (segOuterR + innerR) / 2),
        startAngle,
        math.max(0.001, sweep - 0.04), // 0.04 rad gap between slices
        false,
        paint,
      );
    ...
    // Draw center reticle
    canvas.drawCircle(Offset(cx, cy), 3, Paint()..color = kSecondaryText..style=PaintingStyle.fill);
    canvas.drawCircle(Offset(cx, cy), innerR - 12, Paint()..color = kOutline..style=PaintingStyle.stroke..strokeWidth=1.5);
  }
...
class _InteractiveBarList extends StatefulWidget {
  ...
  final int total;
  const _InteractiveBarList({required this.title, required this.data, required this.total});
...
class _InteractiveBarListState extends State<_InteractiveBarList> {
  ...
  @override
  Widget build(BuildContext context) {
    final sorted = widget.data.toList()..sort((a, b) => b.value.count.compareTo(a.value.count));
    ...
    return Container(
      ...
      child: Column(
        children: sorted.map((e) => _buildBar(e.key, e.value.count, e.value.color)).toList(),
      ),
    );
  }
    ...
    return GestureDetector(
      ...
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(
          ...
          borderRadius: BorderRadius.circular(kRadiusSubtle),
          border: isExpanded ? Border.all(color: kOutline) : Border.all(color: Colors.transparent),
        ),
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
              Flexible(child: Text(label.toUpperCase(),
                  style: GoogleFonts.sora(color: isExpanded ? color : kPrimaryText, fontSize: 13.sp, fontWeight: FontWeight.w700),
                  maxLines: 1, overflow: TextOverflow.ellipsis)),
              Text('$count', style: GoogleFonts.jetBrainsMono(color: isExpanded ? color : kSecondaryText, fontSize: 13.sp, fontWeight: FontWeight.w700)),
            ]),
            SizedBox(height: 8.h),
            LayoutBuilder(builder: (context, constraints) {
              return Stack(children: [
                Container(height: 8.h, width: double.infinity,
                    decoration: BoxDecoration(color: kOutline.withAlpha(150), borderRadius: BorderRadius.circular(kRadiusPill))),
                TweenAnimationBuilder<double>(
                  duration: const Duration(milliseconds: 1000),
                  curve: Curves.easeOutExpo,
            ...
            if (isExpanded) ...[
              ...
              Row(
                mainAxisAlignment: MainAxisAlignment.spaceBetween,
                children: [
                  Text('PORTION OF INVENTORY', style: GoogleFonts.inter(color: kSecondaryText, fontSize: 10.sp, fontWeight: FontWeight.w600)),
                  Text('$pct%', style: GoogleFonts.jetBrainsMono(color: kPrimaryText, fontSize: 12.sp, fontWeight: FontWeight.w700)),
                ],
              ),
            ]
          ],
      ...
      child: Column(
        ...
        children: [
          Padding(
            ...
            child: Text('PRODUCTION TIMELINE', style: GoogleFonts.jetBrainsMono(color: const Color(0xFF4A3510), fontSize: 10.sp, fontWeight: FontWeight.w800, letterSpacing: 1.5)),
          ),
          SizedBox(
            height: 90.h,
      ...
      child: Column(
        ...
        children: [
          Row(
            ...
            children: [
              Text(era.toUpperCase(), style: GoogleFonts.jetBrainsMono(color: kPrimaryText, fontSize: 14.sp, fontWeight: FontWeight.w800)),
              SizedBox(width: 8.w),
              Container(
                padding: EdgeInsets.symmetric(horizontal: 6.w, vertical: 2.h),
                decoration: BoxDecoration(color: kPrimaryText, borderRadius: BorderRadius.circular(4)),
                child: Text('$count', style: GoogleFonts.jetBrainsMono(color: kGold, fontSize: 10.sp, fontWeight: FontWeight.w800)),
              ),
            ],
          ),
          SizedBox(height: 8.h),
          ...
          Row(
            ...
            children: [
              ...
              SizedBox(width: 8.w),
              Container(width: 1.5.w, height: 12.h, color: const Color(0xFF6B4A15)),
              SizedBox(width: 8.w),
              Container(width: 1.5.w, height: 16.h, color: const Color(0xFF6B4A15)),
              SizedBox(width: 8.w),
              Container(width: 1.5.w, height: 12.h, color: const Color(0xFF6B4A15)),
            ],
          ),
